...
const post_products = async (req, res) => {
  try {
    ...
    res.send({ message: "product is added!", item: new_added_product });
  } catch (err) {
 ...
 const put_product = async (req, res) => {
  ...
  if(!product) return res.status(404).send({error:"product not found with that id"});
...
export { get_products, post_products, get_product_by_id, add_review,put_product,delete_products};
